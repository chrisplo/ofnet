...
func testOfnetPortDscpFlow(t *testing.T, agent *OfnetAgent, brName string) {
	...
	// verify port flow
	meta, mask := SrcGroupMetadata(1, 1)
	dstMeta, dstMask := DstGroupMetadata(1, 1)
	portVlanFlowMatch := fmt.Sprintf("priority=10,in_port=14 actions=write_metadata:0x%x/0x%x",
		meta|dstMeta, mask|dstMask)
	if agent.dpName == "vxlan" {
		portVlanFlowMatch = fmt.Sprintf("priority=10,in_port=14 actions=push_vlan:0x8100,set_field:4097->vlan_vid,write_metadata:0x%x/0x%x",
			meta|dstMeta, mask|dstMask)
	}
	...
	// verify dscp v4 flow
	dscpv4FlowMatch := fmt.Sprintf("priority=100,ip,in_port=14 actions=set_field:10->ip_dscp,write_metadata:0x%x/0x%x",
		meta|dstMeta, mask|dstMask)
	if agent.dpName == "vxlan" {
		dscpv4FlowMatch = fmt.Sprintf("priority=100,ip,in_port=14 actions=set_field:10->ip_dscp,push_vlan:0x8100,set_field:4097->vlan_vid,write_metadata:0x%x/0x%x",
			meta|dstMeta, mask|dstMask)
	}
	...
	// verify dscp v6 flow
	dscpv6FlowMatch := fmt.Sprintf("priority=100,ipv6,in_port=14 actions=set_field:10->ip_dscp,write_metadata:0x%x/0x%x",
		meta|dstMeta, mask|dstMask)
	if agent.dpName == "vxlan" {
		dscpv6FlowMatch = fmt.Sprintf("priority=100,ipv6,in_port=14 actions=set_field:10->ip_dscp,push_vlan:0x8100,set_field:4097->vlan_vid,write_metadata:0x%x/0x%x",
			meta|dstMeta, mask|dstMask)
	}
	...
	// verify dscp v4 flow
	dscpv4FlowMatch = strings.Replace(dscpv4FlowMatch, "10->ip_dscp", "20->ip_dscp", 1)
	if !ofctlFlowMatch(flowList, VLAN_TBL_ID, dscpv4FlowMatch) {
	...
	// verify dscp v6 flow
	dscpv6FlowMatch = strings.Replace(dscpv6FlowMatch, "10->ip_dscp", "20->ip_dscp", 1)
	if !ofctlFlowMatch(flowList, VLAN_TBL_ID, dscpv6FlowMatch) {
		fmt.Printf("Flows:\n%v\n", strings.Join(flowList, "\n"))
		t.Fatalf("Could not find the flow %s on ovs %s", dscpv6FlowMatch, brName)
	}
	...
	// verify dscp v4 flow
	dscpv4FlowMatch = strings.Replace(dscpv4FlowMatch, "20->ip_dscp", "30->ip_dscp", 1)
	if !ofctlFlowMatch(flowList, VLAN_TBL_ID, dscpv4FlowMatch) {
		fmt.Printf("Flows:\n%v\n", strings.Join(flowList, "\n"))
		t.Fatalf("Could not find the flow %s on ovs %s", dscpv4FlowMatch, brName)
	}
	...
	// verify dscp v6 flow
	dscpv6FlowMatch = strings.Replace(dscpv6FlowMatch, "20->ip_dscp", "30->ip_dscp", 1)
	if !ofctlFlowMatch(flowList, VLAN_TBL_ID, dscpv6FlowMatch) {
		fmt.Printf("Flows:\n%v\n", strings.Join(flowList, "\n"))
		t.Fatalf("Could not find the flow %s on ovs %s", dscpv6FlowMatch, brName)
	}
